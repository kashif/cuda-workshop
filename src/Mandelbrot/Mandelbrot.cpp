...
void AutoQATest()
{
    if (g_CheckRender && g_CheckRender->IsQAReadback()) {
        ...
        if (g_AutoQuit) {
            ...
            shrQAFinish2(g_AutoQuit, *pArgc, (const char **)pArgv, (g_TotalErrors==0) ? QA_PASSED : QA_FAILED);
            exit(EXIT_SUCCESS);
        }
    }
}
...
void cleanup()
{
    ...
    // We only cleanup if we are running the normal test with visualization
    if (!g_bQAReadback && !g_bPerformanceTest) {
